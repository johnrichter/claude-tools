module github.com/johnrichter/claude-tools
...
require (
	...
	github.com/johnrichter/claude-shared-tooling/go/webfetch v0.0.0
...
// adoption, clikit, fsx, gate, logkit, plugin-foundation, state, sysops, transcript and webfetch
// are ai-shared-lib sibling-repo modules, not yet independently tagged -- this placeholder
// version + local replace is a monorepo-development stand-in a future release transaction
// resolves by cutting real tags and pointing these requires at them. A `replace` directive is
// only honored in the MAIN module's own go.mod, so the full transitive closure is replaced here
// too, even for a module (fsx, gate, logkit) this module never imports directly.
...
replace github.com/johnrichter/claude-shared-tooling/go/webfetch => ../ai-shared-lib/go/webfetch

...
func TestVendoredHookScriptsMatchCanonicalDigest(t *testing.T) {
	if got := sha256Hex(t, "../../hooks/forced-use-hook.sh"); got != canonicalForcedUseHookSHA256 {
		t.Errorf("hooks/forced-use-hook.sh sha256 = %s, want %s (resync from ai-shared-lib/plugin-foundation, don't fork)", got, canonicalForcedUseHookSHA256)
	}
	if got := sha256Hex(t, "../../hooks/download-script.sh"); got != canonicalDownloadScriptSHA256 {
		t.Errorf("hooks/download-script.sh sha256 = %s, want %s (resync from ai-shared-lib/plugin-foundation, don't fork)", got, canonicalDownloadScriptSHA256)
	}
}
